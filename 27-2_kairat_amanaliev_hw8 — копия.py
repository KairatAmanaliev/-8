...
quantity int(5,0)  NOT NULL DEFAULT 0)
'''
...
if conn is not None:
    # add_products(conn, product_list)
        # create_table(conn, sql_create_products_table)
        # insert_product(conn, ('Eggs', 12.2, 100))
        # insert_product(conn, ('Bread', 25, 50))
        # insert_product(conn, ('Butter', 75, 25))
        # insert_product(conn, ('Rice', 75, 30))
        # insert_product(conn, ('Bubblegum', 5, 500))
        # insert_product(conn, ('Tomato', 25.4, 120))
        # insert_product(conn, ('Water', 18.5, 60))
        # insert_product(conn, ('Lemon', 100, 40))
        # select_cheap_products(conn)
        # select_all_products(conn)
        select_product_by_like(conn, 'Soap')
